...
As illustrated above we utliized 5 models for our recommender system with differing RMSE. In our first model (Average Movie Rating), we simply used the average rating as a predictor of actual rating. This produced the highest RMSE of approximatly 1.0612. Next we adjusted the model to account for the movie effect which produced an RMSE of about 0.9439. We further augmented the model by capturing the user effect which gave us a better RMSE of approximatly 0.8653. We then regularized the movie and user effect model to get rid of noisey estimates affectng our model. This led to a slighly better RMSE of about 0.8648. Finallly, by applying matrix factorization we achieveed the best RMSE of 


# Conclusion

The goal of this project was to develop a recommender system. After some exploratory data analysis we realised patterns in our movie and user data that influenced the creation and augmentation of our models. These where the average movie rating model, the movie effect model, the movie and user effect model, the regularized movie and user effect model and finally the matrix factorization which produceded the lowest RMSE of about 0.7939 for our recommendation system.
There were sever CPU and memory limitations, curtailing the utilization of powerful techniques and larger datasets which may have resulted in better model performance.
In the future, it would be intersting to explore how the genre effect may affect model performance. The ensemble method which combines several models could also be employed and its impact to model performance measured.
