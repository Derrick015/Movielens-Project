...
```
After this we will transform them into matrix form.

```{r, echo = TRUE}
edx_fc <- as.matrix(edx_factorization)
validation_fc <- as.matrix(validation_factorization)
```
We will then write these datasets onto he hard disk
